...
Cada ejecución deja constancia en history.log, pase algo o no.
"""
...
def handle_empty_confirmed(state: dict, shelly_status: dict) -> dict:
    ...
    for device_id, was_on in state["splits_on_before"].items():
        if was_on and str(device_id) in by_id:
            ...
            melcloud.set_power(context_key, int(device_id), d["building_id"], power_on=True)
...
def main() -> None:
    ...
    history_line = (
        f"{now().isoformat()} raw={raw} power={shelly_status['power_w']:.1f}W "
        f"relay_on={shelly_status['relay_on']} state={state['confirmed_state']} "
        f"action={state['action']}"
    )
    commit_message = message or "log: sin cambios de estado"

    state_store.save_and_commit(state, history_line, commit_message)
